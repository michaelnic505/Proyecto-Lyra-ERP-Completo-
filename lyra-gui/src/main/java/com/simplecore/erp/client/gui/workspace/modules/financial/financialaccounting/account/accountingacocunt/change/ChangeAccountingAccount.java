...
public class ChangeAccountingAccount extends JPanel implements TransactionPanel, TaskPanel {
    ...
    public ChangeAccountingAccount(int modelId,int classId,AccountingAccountDTO accountingAccount) {
    ...
    private void selectClassItem() {
        for (int i = 0; i < accountClassCombo.getItemCount(); i++) {
            AcModComboItem item = accountClassCombo.getItemAt(i);
            if (!item.getDescription().isEmpty() && item.getId() == classId) {
                accountClassCombo.setSelectedIndex(i);
                break;
            }
        }
    }

    private void selectSubclassItem() {
        for (int i = 0; i < subclassCombo.getItemCount(); i++) {
            AcModComboItem item = subclassCombo.getItemAt(i);
            AccountSubclassDTO subclass = (AccountSubclassDTO) item.getObjectClass();
            if (subclass!=null && subclass.getSubclassId() == accountingAccount.getSubclassId()) {
                subclassCombo.setSelectedIndex(i);
                break;
            }
        }
    }
    ...
    private void selectParentItem() {
        if (accountingAccount.getParentAccountId() != null) {
            for (int i = 0; i < parentAccountCombo.getItemCount(); i++) {
                AcModComboItem item = parentAccountCombo.getItemAt(i);
                AccountingAccountDTO accountingAcc = (AccountingAccountDTO) item.getObjectClass();
                if (accountingAcc!=null && Objects.equals(accountingAcc.getAccountId(), accountingAccount.getParentAccountId())) {
    ...
    private void initializeStatusCombo(){
        ...
        String status = accountingAccount.getAccountStatus();
    ...
    private void setAccountValues() {
        ...
        String accountNumber = accountingAccount.getAccountCode();
        accountNumberTf.setText(accountNumber);

        String accountName = accountingAccount.getAccountName();
        accountNameTf.setText(accountName);

        String description = accountingAccount.getAccountDescription();
        descriptionTextArea.setText(description);

        boolean isClosed = accountingAccount.isClosed();
        isClosedCheckbox.setSelected(isClosed);

        String createdBy = accountingAccount.getCreatedBy();
        createdByTextField.setText(createdBy);

        String createdAt = accountingAccount.getCreatedAt().toString();
        createdAtTextField.setText(createdAt);

    }

    private AccountingAccountDTO preparedAccountingAccount() {
        ...
        // Recoger los valores de los componentes
        int accountId = accountingAccount.getAccountId();
        String accountName = accountNameTf.getText().trim();
        String accountDescription = descriptionTextArea.getText().trim();
        String status = getSelectedAccountStatus();
        boolean isClosed = isClosedCheckbox.isSelected();
        String updatedBy = activeSession.getUsername();

        // Preparar la cuenta para ser modificada
        return buildAccountingAccount(accountId, accountName, accountDescription, status, isClosed, updatedBy);
    }

    private String getSelectedAccountStatus() {
        // Método auxiliar para obtener el nombre del estado seleccionado
        String selectedStatus = statusCombo.getSelectedItem().toString();
        return !selectedStatus.isEmpty() ? selectedStatus : AccountStatus.ACTIVE.name(); // Devuelve un valor por defecto si no hay estado seleccionado
    }

    private AccountingAccountDTO buildAccountingAccount(int accountId,String accountName, String accountDescription,
            String status, boolean isClosed, String updatedBy) {
        // Construir la cuenta de contabilidad con los datos proporcionados
        AccountingAccountDTO changeAccount = new AccountingAccountDTO.Builder()
                .accountId(accountId)
    ...
    private void sendAccountingAccountChanges() {
        try {
            String sessionId = activeSession.getSessionId();
            int userId = activeSession.getUserId();
            AccountingAccountDTO changesAccount = preparedAccountingAccount();
            AccountingAccountChangeRequest changeRequest = new AccountingAccountChangeRequest(sessionId, userId, changesAccount);

            Object response = serverController.sendRequest(changeRequest);
            String code = accountNumberTf.getText();

            if (response instanceof AccountingAccountChangeResponse changeResponse) {
                ...
                if (changeResponse.wasUpdated()) {
                    notificator.showSuccessMsg(AppMessages.msg(AppMessages.Key.UPDATED) + " " + code);
                } else {
                    notificator.showWarningMsg(AppMessages.msg(AppMessages.Key.NOT_UPDATED) + " " + code);
                }
            }
        } catch (IOException | ClassNotFoundException ex) {
            Logger.getLogger(ChangeAccountingAccount.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    private ActionListener setSaveButtonListener() {
        return (ActionEvent e) -> {
            sendAccountingAccountChanges();
        };
    }
    ...
    private void initComponents() {
        ...
        moduleTitleLabel.setText("Changing Accounting Account");
        ...
        authorPanelLayout.setHorizontalGroup(
            ...
            .addGroup(authorPanelLayout.createSequentialGroup()
                ...
                .addGap(0, 0, 0)
                .addComponent(createdAtTextField, javax.swing.GroupLayout.PREFERRED_SIZE, 323, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        );
    ...
    private javax.swing.JComboBox<AcModComboItem> accountClassCombo;
