...
public class Criticality_getTime {

    public static int getDaysCount(String codigo) {
        ...
        try {
            ...
            String query = SQLKeywords.SELECT.toSQL()
                    + OrderCriticality.DIAS.toString()
            ...
            if (rs.next()) {
                descripcion = rs.getInt(1);
            }
            
            pst.close();
            
        } catch (SQLException ex) {
            Logger.getLogger(Criticality_getTime.class.getName()).log(Level.SEVERE, null, ex);
        }
