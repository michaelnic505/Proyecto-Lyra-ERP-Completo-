...
public class UsagesWindowList extends javax.swing.JDialog {
    
    private LanguageManager languageManager;
    private JButton button;
    private TableRowSorter<DynamicTableModel> filter;
    
    public UsagesWindowList(Frame parent, JButton button, LanguageManager languageManager) {
...
// Overloaded method for a JTextField and a JLabel
    ...
    private void applyTranslations(LanguageManager lm) {
        ...
        String title = LyraWorkspace.getTranslator().getTranslation("UsagesWindowList.title");
        windowTitleLabel.setText(title);

    }

    private String[] getTableColumnsName() {
        String key = LyraWorkspace.getTableTranslator().getTranslation("column.usageKey");
        String usageDescription = LyraWorkspace.getTableTranslator().getTranslation("column.usageDescription");
    ...
    private void setTableModel() {
        ...
        String language = languageManager.getLocale().getLanguage();

        String[] columns = getTableColumnsName();

        RoutineSheetsUsage[] rows = RoutineSheetsUsage.values();
        DynamicTableModel model = new DynamicTableModel(rows.length, columns);

        for (int i = 0; i < rows.length; i++) {
            model.setValueAt(rows[i].getId(), i, 0);
            model.setCellEditable(i, 0, false);

            model.setValueAt(rows[i].getDescription(), i, 1);
